"""
Ablation B - Heterogeneity Analysis (F1)
=====================================================
...
Inputs
------
- evaluation_config_F1_L30.pkl    : window definitions & metadata
- dataset_F1_L30.npz              : full target / asset-id arrays
- sep_deep_F1_L30_cpupar/         : per-window separate-model predictions
- joint_mtl_F1_L30/               : per-window joint-model predictions
...
NPZ_DATASET = "dataset_F1_L30.npz"

SEP_DIR = Path("sep_deep_F1_L30_cpupar")
JOINT_DIR = Path("joint_mtl_F1_L30")

OUT_DIR = Path("heterogeneity_analysis_F1_L30")
...
# ============================================================
def main():
    print("=== Ablation B: Heterogeneity Analysis (Who Benefits?) ===\n")
    ...
    # Auto-scale y-axis label when values are very small
    max_abs = np.abs(results_df["mean_delta_mse"].values).max()
    exp_power = int(np.floor(np.log10(max_abs))) if 0 < max_abs < 1e-2 else 0
    scale = 10 ** (-exp_power) if exp_power != 0 else 1.0
    ...
    ylabel = (
        ...
        if exp_power != 0 else "ΔMSE  (Joint − Separate)"
    )
    ax.set_ylabel(ylabel, fontsize=11, color="#374151", labelpad=10)
    ax.set_xlabel(
